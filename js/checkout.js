/* ── checkout.js — Delivery Method + PPN 12% + Voucher Support ── */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    cart.forEach(item => {
        ...
        list.innerHTML += `
            <div class="flex gap-3 mb-3 pb-3" style="border-bottom: 1px solid var(--border-color);">
                ...
                <div>
                    <h4 style="font-size:15px;margin-bottom:4px">${safeName}</h4>
                    <p class="text-muted" style="font-size:13px">${item.qty} &times; Rp ${item.price.toLocaleString()}</p>
                    <p style="font-weight:600;margin-top:4px">Rp ${itemTotal.toLocaleString()}</p>
                </div>
            </div>`;
    });
    ...
    try {
        ...
        const el = document.getElementById('checkout-wallet-balance');
        if (el) el.innerText = `Rp ${data.balance.toLocaleString()}`;
    } catch(e) {}
    ...
    try {
        ...
        if (hint && vouchers.length > 0) {
            hint.innerHTML = '💡 Available: ' + vouchers.map(v =>
